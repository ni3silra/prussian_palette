...
export default function ArtworkCard({ 
  ...
  return (
    <>
      <div className="group w-full relative aspect-[4/5] rounded-3xl overflow-hidden shadow-xl border border-white/50 glass-panel">
        {/* Base Image */}
        <Image 
          src={images[0]} 
          alt={title} 
        ...
        <div className="absolute inset-0 bg-prussian/80 opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center p-6 text-center z-10">
          ...
          <div className="flex flex-col gap-4 w-full max-w-[200px] transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500 delay-150">
            <button 
              ...
              className="w-full px-8 py-3 text-[10px] font-medium uppercase tracking-[0.2em] bg-transparent border border-white/40 text-white rounded-full hover:bg-prussian hover:border-prussian transition-all shadow-lg hover:shadow-2xl"
            >
              View Artwork
            </button>
            <button className="w-full px-8 py-3 text-[10px] font-medium uppercase tracking-[0.2em] bg-transparent border border-white/40 text-white rounded-full hover:bg-prussian hover:border-prussian transition-all shadow-lg hover:shadow-2xl">
              Own
            </button>
          </div>
        </div>
      ...
      {isMounted && (
        ...
        >
          ...
          >
            ...
            <button 
              onClick={() => setIsModalOpen(false)}
              className="absolute top-4 right-4 z-50 w-10 h-10 flex items-center justify-center bg-white/50 hover:bg-white text-prussian rounded-full backdrop-blur-md transition-all shadow-sm group/close"
            ...
            <div className="w-full md:w-[60%] h-[40vh] md:h-[90vh] bg-transparent relative">
              <Swiper
                ...
                style={{
                  '--swiper-navigation-color': '#223240',
                  '--swiper-pagination-color': '#223240',
            ...
            <div className="w-full md:w-[40%] flex flex-col justify-center p-6 md:p-10 h-[50vh] md:h-[90vh] overflow-y-auto custom-scrollbar">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-6 h-[1px] bg-prussian/30"></div>
                <p className="font-accent text-[10px] tracking-[0.4em] uppercase text-prussian-lt">Original Collection</p>
              </div>
              
              <h2 className="font-display text-3xl md:text-4xl text-prussian mb-2 leading-tight">{title}</h2>
              
              <p className="font-light text-sm text-prussian-lt mb-6">{size} · {price}</p>
              
              <div className="prose prose-sm prose-prussian mb-8 font-body font-light text-prussian/80 leading-relaxed whitespace-pre-line">
                {description}
              </div>
              
              <div className="flex flex-col sm:flex-row gap-4">
                <button className="w-full sm:w-auto px-8 py-3 text-[10px] font-medium uppercase tracking-widest bg-prussian text-white rounded-full hover:bg-prussian-lt transition-all shadow-xl hover:shadow-2xl flex-shrink-0 text-center">
                  Own This Piece
                </button>
                <button className="w-full sm:w-auto px-8 py-3 text-[10px] font-medium uppercase tracking-widest bg-transparent border border-prussian/30 text-prussian rounded-full hover:bg-prussian hover:text-white hover:border-prussian transition-all shadow-sm hover:shadow-lg flex-shrink-0 text-center">
